...
```bash
node .claude/skills/gcp-diagram/scripts/build-diagram.js               # 純現況版（預設）→ report/gcp-architecture.drawio
node .claude/skills/gcp-diagram/scripts/build-diagram.js --annotated   # 詳註版（含 ⚠ 稽核註記與網路模型解說）
```

產出多分頁 `.drawio`（全景架構／架構索引／每個有工作負載的 VPC 網路一頁），
用 [app.diagrams.net](https://app.diagrams.net) 或 VS Code 的 Draw.io 擴充開啟。
**預設為「純現況版」**：只畫拓撲與組態（名稱／IP／CIDR／版本／防火牆規則名／連線），
不寫 ⚠ 評語與解說（顏色視覺仍標示暴露）；稽核結論看報告。`--annotated` 回到含註記的詳註版。
