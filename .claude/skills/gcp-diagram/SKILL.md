...
1. 執行產生器（從專案根目錄）：
   ```
   node .claude/skills/gcp-diagram/scripts/build-diagram.js               # 預設：純現況版
   node .claude/skills/gcp-diagram/scripts/build-diagram.js --annotated   # 詳註版（含稽核註記與解說）
   ```
   選用 `--out <路徑>` 改輸出位置（預設 `report/gcp-architecture.drawio`）。

   **兩種輸出模式（差別只在文字，拓撲與顏色視覺一致）**：
   - **純現況版（預設）**：只畫純拓撲與組態——名稱／IP（私有＋公開）／機型／CIDR／版本／防火牆規則名清單／
     連線。**不寫** ⚠ 評語旗標（SSL 未強制、未備份、存取記錄未開、未掛 Armor、節點非私有、
     VM 對 0.0.0.0/0 開放…）與 GCP 網路模型的教學式解說。紅／藍通道與防火牆紅框的**顏色**仍在
     （視覺標示暴露），只是不附評語字。完整性標記（未掃描到的群組、摺疊子網數、資料缺口）保留（精簡版）。
     適合當「實際狀態架構圖」對外呈現。**稽核結論一律回歸報告（`findings/`／`report/`），不重複寫進圖。**
   - **詳註版（`--annotated`）**：回到含 ⚠ 稽核註記與網路模型解說的版本，供內部覆核／教學用。
   - 兩種模式由 `build-diagram.js` 內的 `ANNOTATED`（讀 `--annotated`）與 helper `A()` 統一控制——
     要調整「哪些算評語、哪些算現況」改對應 label 函式，不要在產出的 `.drawio` 上手改。
...
> 以下描述的**版面／分頁／防火牆框與通道**兩種模式一致；文中提到的 ⚠ 評語與解說字（如「明說無對外路徑」、
> 「標出規則名與埠」）是**詳註版（`--annotated`）**的內容，**純現況版（預設）會精簡或省略這些文字**（顏色視覺仍在）。
...
> **這些 ⚠ 文字只在詳註版（`--annotated`）輸出。** 純現況版（預設）不寫 ⚠ 評語字，但**顏色視覺仍在**
> （所在資源列轉黃框、防火牆框轉紅）——判定邏輯（`ruleApplies`／`exposed`／`openToWorld`）兩版都跑、
> 都驅動顏色，差別只在「要不要把評語寫成文字」。稽核結論以報告（`findings/`／`report/`）為準。
