...
#    不要在這裡新增 bash 產表段落（AWS 版就是這樣長成多語言拼接的）。
...
# ── 成本訊號表：GCP 沒有 Cost Explorer 明細 API，成本支柱靠這張表 ──────
...
#    必須照實寫進報告，不可用推算數字冒充實際帳單。
{
  echo "# 成本訊號表"
  echo ""
  echo "GCP **沒有**等同 AWS Cost Explorer 的成本明細 API——「各服務各月花多少錢」需要"
...
# 這類機械性比對不該交給 LLM 判斷——AWS 版就曾因為漏做這一步，
