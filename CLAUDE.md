...
① gcp-scanner（同步執行）
     ...
     scan.sh 末尾自動跑 digest.sh → data/digest/（精簡投影＋跨檔關聯事實表＋覆蓋率檢查 coverage-gaps.md）
...
| `.claude/skills/report-gcp/scripts/scan.sh` | 唯讀掃描腳本（scanner 執行；`scan.sh <project-id> <期別>`）。標準服務的掃描清單抽到 `references/scan-manifest.json`，由內建 manifest 引擎逐 section 執行（動詞限唯讀白名單＋依 api 對 services-enabled.json 做啟用預檢）；行為不標準的服務（describe 迴圈／多 region 合併／stderr 污染／回應非 `[]`）仍以程式碼留在本檔 |
...
| `.claude/skills/report-gcp/scripts/coverage-check.sh` | 覆蓋率檢查（純諮詢）：以 Cloud Asset Inventory（`data/global/asset-inventory.json`）diff `scan-manifest.json` 宣告的覆蓋（COVERED＝manifest 各條 assetTypes 聯集，單一真相來源），確定性標出「有資源卻沒掃」的缺口 → `data/digest/coverage-gaps.md`；未知 assetType 一律歸「待分類缺口」（未來新服務自動浮現）。**只報告、不阻斷**（不 exit 1）；覆蓋回歸靠人每期檢視本報告。digest.sh 末尾呼叫 |
...
| `.claude/skills/report-gcp/references/scan-manifest.json` | 掃描清單（進版控、受審；單一真相來源）。可執行條目帶 `args`＋`api`＋`assetTypes`，manifest 引擎執行；`handledIn=scan.sh` 條目只宣告 `assetTypes`（該掃描由 scan.sh 程式碼負責，通常因 describe 迴圈耦合或結構性例外）。coverage-check 的 COVERED＝所有條目 assetTypes 聯集。新增標準服務只改此檔一處。`args` 不含 `--project`（引擎自動附加） |
