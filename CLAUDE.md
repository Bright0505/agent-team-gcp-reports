...
| `.claude/skills/report-gcp/scripts/coverage-check.sh` | 覆蓋率檢查：以 Cloud Asset Inventory（`data/global/asset-inventory.json`）diff scan.sh 已知覆蓋清單，確定性標出「有資源卻沒掃」的缺口 → `data/digest/coverage-gaps.md`；未知 assetType 一律歸「待分類缺口」（未來新服務自動浮現）。讀 `required-coverage.json` 執「必掃」政策：必掃型別若存在卻未覆蓋則硬失敗（exit 1）擋下 digest。digest.sh 末尾呼叫 |
...
| `.claude/skills/report-gcp/references/required-coverage.json` | 必掃清單（政策斷言，不執行任何 gcloud，**進版控**以留稽核軌跡）：列出的 assetType 若存在於專案卻未被 scan.sh 掃到，`coverage-check.sh` 硬失敗擋下 digest。用途：保護 findings 依賴的關鍵型別不因重構無聲掉出覆蓋、並作為「新增必掃」的執法點 |
