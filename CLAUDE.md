...
> 本專案的流程骨架、檔案介面與省 token 規則移植自姊妹專案 `agent-team-aws-reports`，
> 其中多數規則是那邊踩過坑後固化下來的機制（下方標「AWS 版教訓」者即是）。
> GCP 的資源階層、網路模型與成本資料可得性與 AWS 不同，掃描與跨檔關聯是重新設計的。
...
- **dontAsk 的語意是「不問＝自動拒絕」**：allow 內靜默放行、deny 內擋下、兩者皆非→自動拒。
  ...
  腳本時，**必須同步加 allow 條目**（AWS 版教訓：曾因 archive-report.sh 漏加而在無人值守中被拒，
...
## GCP 特有的三件事（與 AWS 版最大的差異）

1. **成本明細查不到**：GCP 沒有等同 Cost Explorer 的 API，成本明細需 BigQuery 帳單匯出。
   本專案不接，因此**實際帳單明細一律列為資料缺口**，成本支柱改以
   「預算設定 ＋ Recommender 官方估算 ＋ 資源組態推算」為依據。
   **絕不可用推算值冒充實際帳單數字。**
2. **沒有公有／私有子網**：GCP 的對外可及性看「VM 有沒有外部 IP」、對外連出看 Cloud NAT、
   存取 Google API 看 Private Google Access。照搬 AWS 的公私子網心智模型會得到錯誤結論。
...
- **機械性的跨檔比對交給腳本，不要交給 LLM 判斷**：像「這條 0.0.0.0/0 的防火牆規則到底有沒有
  ...
  LLM 會忘、會隨機（AWS 版教訓：曾因漏做這一步把 [高] 發現降級成 [中]，還給出該環境做不到的修復建議）。
...
- **連結一律取自 `.claude/skills/report-gcp/references/gcp-docs-<支柱>.md`**（依支柱拆分，各 agent 只讀自己那份），
  ...
  查完把新連結補回**對應支柱的檔案**。例外：成本金額估算需要當下單價時，仍應 WebFetch 定價頁。
